        ...
        if f:
            if len(exist_ids) == 0:
                ...
                for nid in new_ids:
                    ...
                    for oid in (exist_ids - set(unpickable)) & set(final_fuse_id.keys()):
                        try:
                            if face_feats[nid] != None and face_feats[oid] != None:
                                ...
                                face_dis.append([oid, tmp])
                            try:
                                if body_feats[nid] != None and body_feats[oid] != None:
                                    tmp = np.mean(reid.compute_distance(body_feats[nid], body_feats[oid]))
                                    print('body nid {}, oid {}, tmp {}'.format(nid, oid, tmp*0.01))
                                    body_dis.append([oid, tmp*0.01])
                            except KeyError as e:
                                print(f"Body KeyError: {e} not found in face_feats dictionary")
                                body_dis.append([0, np.inf])
                                
                        except KeyError as e:
                            print(f"Face KeyError: {e} not found in face_feats dictionary")
                            face_dis.append([0, np.inf])
                    ...
                    face_dis = list(filter(lambda x: x[1] < face_threshold, face_dis))
                    body_dis = list(filter(lambda x: x[1] < body_threshold, body_dis))
                    
                    print("FACE_DIS :",face_dis)
                    print("Body_DIS :",body_dis)
                    # print(f"Body Distance between {dis[0][]} and {nid} is : {dis[0][1]}");

                    # if face feature is none and body feature is not none:
                
                    if len(face_dis) == 0:
                        print("##NO FACE##")
                        if not body_dis:
                            final_fuse_id[nid] = [nid]
                            print("##NO BODY## added new id:",nid)
                            continue
                        if len(body_dis) > 0 and body_dis[0][1] < body_threshold:
                            combined_id = body_dis[0][0]
                            images_by_id[combined_id] += images_by_id[nid]
                            final_fuse_id[combined_id].append(nid)
                            print(f"***new id : {nid} combined with oid : {combined_id}")
                
                            
                    # If both face feature and body feature
                    elif face_dis[0][1] < face_threshold:
                       
                        # Filter out the faces below the threshold
                        

                        # Sort the face distances in ascending order
                        face_dis.sort(key=operator.itemgetter(1))
                        print(f"*face oid {face_dis[0][0]} and face distance {face_dis[0][1]}")
                        combined_id = face_dis[0][0]
                        images_by_id[combined_id] += images_by_id[nid]
                        final_fuse_id[combined_id].append(nid)
                        print(f"***new id : {nid} combined with oid : {combined_id}")
                        # if len(body_dis) == 0:
                        ...
                        #     print(f"***new id : {nid} combined with oid : {combined_id}")
                        # else:
                        #     print("## DETECTED FACE AND BODY ##")
                        #     # Create a dictionary to store the corresponding body distances of the filtered face distances
                        #     body_dict = {item[0]: item[1] for item in body_dis if item[0] in [f[0] for f in face_dis]}

                        #     # Sort the body distances in ascending order
                        #     body_dis.sort(key=operator.itemgetter(1))
                        #     print(f"*Body oid {body_dis[0][0]} and body distance {body_dis[0][1]}")
                        #     combined_id = body_dis[0][0]
                        #     images_by_id[combined_id] += images_by_id[nid]
                        #     final_fuse_id[combined_id].append(nid)
                        #     print(f"***new id : {nid} combined with oid : {combined_id}")
                       
                    # apply body feature above face threshold
                    elif len(body_dis) > 0 and body_dis[0][1] < body_threshold:
                        print("BODY")
                        combined_id = body_dis[0][0]
                        images_by_id[combined_id] += images_by_id[nid]
                        final_fuse_id[combined_id].append(nid)
